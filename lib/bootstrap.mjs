...
function takeLastImages(maxAgeMs = 6e4) {
...
function apply4(ctx2, config) {
  ...
  ctx2.provide("mcMemos", service);
  const currentUsername = () => ctx2.mcbot?.username ?? "";
...
function apply5(ctx2, config) {
  ...
  const statePath = resolve3(config.statePath);
...
${loreHits.slice(0, 6).map((m) => `- ${m}`).join("\n")}` : "",
    ...
    const resolve11 = (out.resolve ?? "").trim();
    let memoSaved = false;
    if (growth || resolve11) {
      const id = await ctx2.mcMemos.remember(username, `\u3010\u6E38\u620F\u591C\u8FDB\u5316\u3011${growth}${resolve11 ? ` \u660E\u65E5\u4E4B\u5FC3\uFF1A${resolve11}` : ""}`);
      memoSaved = id !== null;
    }
    return `ok: \u590D\u76D8 \u4E2A\u4EBA${memories.length}+\u4E16\u754C${loreHits.length} \u6761 \u2192 ${lessonCount} \u5F20\u65B0\u6559\u8BAD\u5361${memoSaved ? " + \u6210\u957F\u5DF2\u5165\u957F\u7EBF\u8BB0\u5FC6" : ""}\uFF1B\u51B3\u5FC3\uFF1A${resolve11.slice(0, 40) || "(\u672A\u7ED9)"}`;
  }
...
function apply6(ctx2, config) {
  ...
  const hotspotsPath = join2(dataDir, "death-hotspots.json");
  const proposalsDir = join2(dataDir, "evolution-proposals");
  const directivesPathOf = (u) => join2(dataDir, `evolution-directives-${u}.json`);
  const tuningPathOf = (u) => join2(dataDir, `self-tuning-${u}.json`);
  ...
  ctx2.tools.register(defineTool3({
    ...
    execute: async (args) => {
      ...
      saveJson(join2(proposalsDir, `${id}.json`), p);
      try {
...
var TransmigratorRegistry = class {
  ...
  constructor(registryPath) {
    const path = resolve4(registryPath);
  ...
  readText(base, rel, fallback) {
    ...
    try {
      const p = resolve4(base, rel);
      if (existsSync4(p)) return readFileSync5(p, "utf-8").trim();
    } catch {
    }
    return fallback;
  }
...
function apply8(ctx2, config) {
  ...
  const statePath = resolve5(config.seedStatePath);
  const service = {
    anchor(username) {
  ...
  async function seedAll() {
    let state = {};
    try {
      if (existsSync5(statePath)) state = JSON.parse(readFileSync6(statePath, "utf-8"));
    } catch {
      state = {};
    }
...
var MysticStore = class {
  ...
  constructor(path) {
    ...
    try {
      if (existsSync6(this.path)) {
        const raw = JSON.parse(readFileSync7(this.path, "utf-8"));
  ...
  persist() {
    try {
      mkdirSync5(dirname5(this.path), { recursive: true });
      const tmp = this.path + ".tmp";
...
import { appendFileSync, existsSync as existsSync7, mkdirSync as mkdirSync6, readFileSync as readFileSync8 } from "node:fs";
import { dirname as dirname6, join as join3, resolve as resolve7 } from "node:path";
var name10 = "mc-wiki";
var inject9 = [];
...
var WikiStore = class {
  ...
  constructor(dataDir, username, maxCards) {
    mkdirSync6(dirname6(join3(dataDir, "x")), { recursive: true });
    this.path = join3(dataDir, `wiki-${username}.jsonl`);
    this.maxCards = maxCards;
    if (existsSync7(this.path)) {
      for (const line of readFileSync8(this.path, "utf-8").split("\n")) {
        const t = line.trim();
        if (!t) continue;
        try {
...
function apply10(ctx2, config) {
  const dir = resolve7(config.dataDir);
...
function apply11(ctx2, config) {
  ...
  const villageDir = resolve8(config.dataDir, "village");
  let villagers = [];
  let quests = [];
  let cacheAt = 0;
  function refresh() {
    ...
    cacheAt = Date.now();
    try {
      const raw = JSON.parse(readFileSync9(resolve8(villageDir, "villagers.json"), "utf-8"));
    ...
    try {
      ...
      const p = resolve8(villageDir, `quests-${day}.json`);
      if (existsSync8(p)) {
...
import { appendFileSync as appendFileSync2, mkdirSync as mkdirSync7, readFileSync as readFileSync10, readdirSync, writeFileSync as writeFileSync6 } from "node:fs";
import { dirname as dirname7, join as join4, resolve as resolve9 } from "node:path";
...
function apply12(ctx2, config) {
  ...
  const brainLogPath = resolve9(config.brainLogPath);
  const statusDir = dirname7(resolve9(config.statusPath));
  let disposed = false;
  let busy = false;
  let steps = 0;
  let lastAction = "none";
  ...
  const episodicPath = (username) => join4(statusDir, `episodic-${username}.jsonl`);
  function restoreEpisodic(username) {
    episodicRestored = true;
    let lines = [];
    try {
      lines = readFileSync10(episodicPath(username), "utf8").split("\n").filter(Boolean);
    } catch {
      return;
    }
  ...
  function appendEpisodic(username, text6) {
    try {
      appendFileSync2(episodicPath(username), JSON.stringify({ ts: (/* @__PURE__ */ new Date()).toISOString(), text: text6 }) + "\n", "utf8");
    } catch (err) {
  ...
  let frozenSteps = 0;
  const defectDir = resolve9(config.defectDir);
  let streakTool = "";
  let streakCount = 0;
  const streakSamples = [];
  function stampNow() {
  ...
  function findOpenTicket(tool) {
    ...
    for (const f of files) {
      try {
        const t = JSON.parse(readFileSync10(join4(defectDir, f), "utf8"));
        if (t.tool === tool && t.status !== "closed") return { path: join4(defectDir, f), ticket: t };
      } catch {
      }
    }
    return null;
  ...
  function fileDefectTicket(bot, tool, goal) {
    try {
      mkdirSync7(defectDir, { recursive: true });
      ...
      const path = join4(defectDir, `${ticket.id}.json`);
...
\u8BF7\u603B\u7ED3\u8FD9\u4E2A\u76EE\u6807\u4E3A\u4EC0\u4E48\u53CD\u590D\u5931\u8D25\uFF0C\u4E0B\u6B21\u5E94\u8BE5\u6362\u4EC0\u4E48\u505A\u6CD5\uFF08\u6362\u8DEF\u7EBF/\u6362\u5DE5\u5177/\u5148\u505A\u51C6\u5907/\u653E\u5F03\u6B64\u76EE\u6807\uFF09\u3002`);
  ...
  function writeMapSnapshot(bot) {
    try {
      ...
      writeFileSync6(
        join4(statusDir, `map-${username}.json`),
        JSON.stringify({
  ...
  function writeStatus(bot) {
    try {
      ...
      writeFileSync6(
        join4(statusDir, `status-${username}.json`),
        JSON.stringify({
          updatedAt: (/* @__PURE__ */ new Date()).toISOString(),
          bot: {
            online: true,
  ...
  function brainLog(entry) {
    try {
      appendFileSync2(brainLogPath, JSON.stringify(entry) + "\n", "utf-8");
    } catch (err) {
  ...
  async function step() {
    ...
    try {
      ...
      if (process.env.MC_EYES_SHOT !== "0") {
        try {
          const shot = await captureFirstPerson(bot, join4(statusDir, "screenshots"));
          autoShot = shot.file;
        } catch {
        }
      }
...
import { closeSync, existsSync as existsSync9, openSync, readSync, readdirSync as readdirSync2, readFileSync as readFileSync11, statSync } from "node:fs";
import { join as join5, resolve as resolve10, sep } from "node:path";
var name13 = "mc-panel";
var inject12 = [];
...
function resolveUsername(dataDir, configured) {
  ...
  try {
    ...
    for (const f of readdirSync2(dataDir)) {
      ...
      if (!m) continue;
      const mt = statSync(join5(dataDir, f)).mtimeMs;
      if (mt > bestMtime) {
        bestMtime = mt;
        best = m[1];
      }
...
function resolveShotPath(dataDir, shot) {
  if (!shot) return null;
  const root = resolve10(dataDir, "screenshots");
  const full = resolve10(root, shot);
...
function collect(dataDir, username) {
  ...
  if (!u) return payload;
  const statusRaw = readJson(join5(dataDir, `status-${u}.json`));
  ...
  const tr = readJson(join5(dataDir, "transmigrators.json"));
  ...
  const mystic = readJson(join5(dataDir, "mystic-state.json"));
  const players = mystic?.players ?? {};
  if (players[u]) payload.mystic = { innateSkill: players[u].innateSkill, level: players[u].level };
  try {
    const lines = readFileSync11(join5(dataDir, `wiki-${u}.jsonl`), "utf-8").split("\n").filter(Boolean);
  ...
  payload.memory = readJson(join5(dataDir, "mc-memory.json"));
  try {
    const ddir = join5(dataDir, "defects");
    for (const f of readdirSync2(ddir)) {
      if (!f.endsWith(".json")) continue;
      const d = readJson(join5(ddir, f));
      if (!d) continue;
  ...
  payload.topo = readJson(join5(dataDir, `map-${u}.json`));
  try {
    const raw = tailFile(join5(dataDir, `episodic-${u}.jsonl`), 64 * 1024);
...
function apply13(ctx2, config) {
  if (!config.enabled) return;
  const dataDir = resolve10(config.dataDir);
...
if (RUN_MS > 0) {
  await new Promise((resolve11) => setTimeout(resolve11, RUN_MS));
