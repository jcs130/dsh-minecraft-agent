...
import { existsSync as existsSync3, mkdirSync as mkdirSync3, readFileSync as readFileSync4, appendFileSync, readdirSync } from "node:fs";
...
var LocalMemoryBackend = class {
  ...
  fileOf(pool) {
    return join4(this.dir, `${pool}.jsonl`);
  }
  readPool(pool) {
...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync3, mkdirSync as mkdirSync4 } from "node:fs";
...
${loreHits.slice(0, 6).map((m) => `- ${m}`).join("\n")}` : "",
    ...
    const resolve13 = (out.resolve ?? "").trim();
    let memoSaved = false;
    if (growth || resolve13) {
      const id = await ctx2.mcMemos.remember(username, `\u3010\u6E38\u620F\u591C\u8FDB\u5316\u3011${growth}${resolve13 ? ` \u660E\u65E5\u4E4B\u5FC3\uFF1A${resolve13}` : ""}`);
      memoSaved = id !== null;
    }
    return `ok: \u590D\u76D8 \u4E2A\u4EBA${memories.length}+\u4E16\u754C${loreHits.length} \u6761 \u2192 ${lessonCount} \u5F20\u65B0\u6559\u8BAD\u5361${memoSaved ? " + \u6210\u957F\u5DF2\u5165\u957F\u7EBF\u8BB0\u5FC6" : ""}\uFF1B\u51B3\u5FC3\uFF1A${resolve13.slice(0, 40) || "(\u672A\u7ED9)"}`;
  }
...
function saveJson(path, data) {
  mkdirSync5(join5(path, ".."), { recursive: true });
...
function apply6(ctx2, config) {
  ...
  const hotspotsPath = join5(dataDir, "death-hotspots.json");
  const proposalsDir = join5(dataDir, "evolution-proposals");
  const directivesPathOf = (u) => join5(dataDir, `evolution-directives-${u}.json`);
  const tuningPathOf = (u) => join5(dataDir, `self-tuning-${u}.json`);
  mkdirSync5(proposalsDir, { recursive: true });
  const msgRing = [];
  let lastDeathAt = 0;
  try {
  ...
  ctx2.tools.register(defineTool3({
    ...
    execute: async (args) => {
      ...
      saveJson(join5(proposalsDir, `${id}.json`), p);
      try {
...
import { existsSync as existsSync6, mkdirSync as mkdirSync6, readFileSync as readFileSync8, renameSync as renameSync2, writeFileSync as writeFileSync5 } from "node:fs";
...
import { existsSync as existsSync7, mkdirSync as mkdirSync7, readFileSync as readFileSync9, renameSync as renameSync3, writeFileSync as writeFileSync6 } from "node:fs";
...
var MysticStore = class {
  ...
  persist() {
    try {
      mkdirSync7(dirname6(this.path), { recursive: true });
      const tmp = this.path + ".tmp";
...
import { appendFileSync as appendFileSync2, existsSync as existsSync8, mkdirSync as mkdirSync8, readFileSync as readFileSync10 } from "node:fs";
import { dirname as dirname7, join as join6, resolve as resolve9 } from "node:path";
var name10 = "mc-wiki";
var inject9 = [];
...
var WikiStore = class {
  ...
  constructor(dataDir, username, maxCards) {
    mkdirSync8(dirname7(join6(dataDir, "x")), { recursive: true });
    this.path = join6(dataDir, `wiki-${username}.jsonl`);
...
function apply11(ctx2, config) {
  ...
  const villageDir = resolve10(config.dataDir, "village");
  let villagers = [];
  let quests = [];
  let cacheAt = 0;
  function refresh() {
    ...
    cacheAt = Date.now();
    try {
      const raw = JSON.parse(readFileSync11(resolve10(villageDir, "villagers.json"), "utf-8"));
    ...
    try {
      ...
      const p = resolve10(villageDir, `quests-${day}.json`);
      if (existsSync9(p)) {
...
import { appendFileSync as appendFileSync3, mkdirSync as mkdirSync9, readFileSync as readFileSync12, readdirSync as readdirSync2, writeFileSync as writeFileSync7 } from "node:fs";
...
function apply12(ctx2, config) {
  ...
  const brainLogPath = resolve11(config.brainLogPath);
  const statusDir = dirname8(resolve11(config.statusPath));
  let disposed = false;
  let busy = false;
  let steps = 0;
  let lastAction = "none";
  ...
  const episodicPath = (username) => join7(statusDir, `episodic-${username}.jsonl`);
  function restoreEpisodic(username) {
    episodicRestored = true;
    let lines = [];
    try {
      lines = readFileSync12(episodicPath(username), "utf8").split("\n").filter(Boolean);
    } catch {
      return;
    }
  ...
  let frozenSteps = 0;
  const defectDir = resolve11(config.defectDir);
  let streakTool = "";
  let streakCount = 0;
  const streakSamples = [];
  function stampNow() {
  ...
  function findOpenTicket(tool) {
    ...
    for (const f of files) {
      try {
        const t = JSON.parse(readFileSync12(join7(defectDir, f), "utf8"));
        if (t.tool === tool && t.status !== "closed") return { path: join7(defectDir, f), ticket: t };
      } catch {
      }
    }
    return null;
  ...
  function fileDefectTicket(bot, tool, goal) {
    try {
      mkdirSync9(defectDir, { recursive: true });
      ...
      const path = join7(defectDir, `${ticket.id}.json`);
...
\u8BF7\u603B\u7ED3\u8FD9\u4E2A\u76EE\u6807\u4E3A\u4EC0\u4E48\u53CD\u590D\u5931\u8D25\uFF0C\u4E0B\u6B21\u5E94\u8BE5\u6362\u4EC0\u4E48\u505A\u6CD5\uFF08\u6362\u8DEF\u7EBF/\u6362\u5DE5\u5177/\u5148\u505A\u51C6\u5907/\u653E\u5F03\u6B64\u76EE\u6807\uFF09\u3002`);
  ...
  function writeMapSnapshot(bot) {
    try {
      ...
      writeFileSync7(
        join7(statusDir, `map-${username}.json`),
        JSON.stringify({
          updatedAt: (/* @__PURE__ */ new Date()).toISOString(),
          r: R,
          cx: Math.floor(p.x),
  ...
  function writeStatus(bot) {
    try {
      ...
      writeFileSync7(
        join7(statusDir, `status-${username}.json`),
        JSON.stringify({
          updatedAt: (/* @__PURE__ */ new Date()).toISOString(),
          bot: {
            online: true,
  ...
  async function step() {
    ...
    try {
      ...
      if (process.env.MC_EYES_SHOT !== "0") {
        try {
          const shot = await captureFirstPerson(bot, join7(statusDir, "screenshots"));
          autoShot = shot.file;
        } catch {
        }
      }
...
import { closeSync, existsSync as existsSync10, mkdirSync as mkdirSync10, openSync, readSync, readdirSync as readdirSync3, readFileSync as readFileSync13, statSync, unlinkSync, writeFileSync as writeFileSync8 } from "node:fs";
import { join as join8, resolve as resolve12, sep } from "node:path";
var name13 = "mc-panel";
var inject12 = [];
...
function readJson(file) {
  try {
    return JSON.parse(readFileSync13(file, "utf-8"));
  } catch {
    return null;
  }
}
function resolveUsername(dataDir, configured) {
  ...
  try {
    ...
    for (const f of readdirSync3(dataDir)) {
      ...
      if (!m) continue;
      const mt = statSync(join8(dataDir, f)).mtimeMs;
      if (mt > bestMtime) {
        bestMtime = mt;
        best = m[1];
      }
...
function resolveShotPath(dataDir, shot) {
  if (!shot) return null;
  const root = resolve12(dataDir, "screenshots");
  const full = resolve12(root, shot);
...
function collect(dataDir, username) {
  ...
  payload.connection.override = readJson(join8(dataDir, CONNECTION_FILE));
  payload.connection.runtime = readJson(join8(dataDir, RUNTIME_FILE));
  if (!u) return payload;
  const statusRaw = readJson(join8(dataDir, `status-${u}.json`));
  ...
  const tr = readJson(join8(dataDir, "transmigrators.json"));
  ...
  const mystic = readJson(join8(dataDir, "mystic-state.json"));
  const players = mystic?.players ?? {};
  if (players[u]) payload.mystic = { innateSkill: players[u].innateSkill, level: players[u].level };
  try {
    const lines = readFileSync13(join8(dataDir, `wiki-${u}.jsonl`), "utf-8").split("\n").filter(Boolean);
  ...
  payload.memory = readJson(join8(dataDir, "mc-memory.json"));
  try {
    const ddir = join8(dataDir, "defects");
    for (const f of readdirSync3(ddir)) {
      if (!f.endsWith(".json")) continue;
      const d = readJson(join8(ddir, f));
      if (!d) continue;
  ...
  payload.topo = readJson(join8(dataDir, `map-${u}.json`));
  try {
    const raw = tailFile(join8(dataDir, `episodic-${u}.jsonl`), 64 * 1024);
...
function apply13(ctx2, config) {
  if (!config.enabled) return;
  const dataDir = resolve12(config.dataDir);
  const server = createServer((req, res) => {
    try {
        ...
        readBody(req).then(
          (body) => {
            try {
              ...
              writeFileSync8(join8(dataDir, CONNECTION_FILE), JSON.stringify(payload, null, 2) + "\n", "utf-8");
      ...
      if (url === "/api/connection/reset" && req.method === "POST" || url === "/api/connection" && req.method === "DELETE") {
        try {
          unlinkSync(join8(dataDir, CONNECTION_FILE));
...
if (RUN_MS > 0) {
  await new Promise((resolve13) => setTimeout(resolve13, RUN_MS));
