...
function setLastImage(dataUrl, file = null, username) {
  const s = slotOf(username ?? DEFAULT_SLOT);
  s.images = [{ dataUrl, file }];
  s.at = Date.now();
}
function setLastImages(images, username) {
    ...
    execute: guard(async (bot, args) => {
      ...
      if (mode === "around") {
        try {
          ...
          setLastImage(dataUrl2, file2, bot.username);
...
var Config3 = Schema3.object({
  memoryPath: Schema3.string().default("./data/mc-memory.json"),
  maxPointsPerType: Schema3.number().default(20)
});
function emptyState() {
...
var Config4 = Schema4.object({
  ...
  backend: Schema4.string().default("auto"),
  baseUrl: Schema4.string().default("http://127.0.0.1:8002"),
  timeoutMs: Schema4.number().default(8e3),
  maxRecall: Schema4.number().default(5),
  localDir: Schema4.string().default("./data/memory"),
  username: Schema4.string().default("")
});
...
var Config5 = Schema5.object({
  ...
  baseUrl: Schema5.string().default("http://localhost:8890/v1"),
  apiKey: Schema5.string().default("sk-local"),
  model: Schema5.string().default("qwen3.8"),
  cooldownHours: Schema5.number().default(12),
  reasoningEffort: Schema5.string().default("xhigh"),
  maxTokens: Schema5.number().default(2048),
  statePath: Schema5.string().default("./data/evolve-state.json"),
  perQuery: Schema5.number().default(8),
  diaryEnabled: Schema5.boolean().default(true),
...
var Config6 = Schema6.object({
  enabled: Schema6.boolean().default(true),
  dataDir: Schema6.string().default("./data"),
  godName: Schema6.string().default("Goddess"),
  clusterSize: Schema6.number().default(8),
  maxHotspots: Schema6.number().default(4),
  maxDirectives: Schema6.number().default(10)
});
var TUNABLE = {
...
var Config7 = Schema7.object({
  registryPath: Schema7.string().default("./data/transmigrators.json")
});
...
var Config8 = Schema8.object({
  seedStatePath: Schema8.string().default("./data/identity-seed.json"),
  maxChunkChars: Schema8.number().default(400),
  seedIntervalMs: Schema8.number().default(300)
});
...
var Config9 = Schema9.object({
  enabled: Schema9.boolean().default(true),
  godName: Schema9.string().default("Goddess"),
  chantTimeoutMs: Schema9.number().default(3e4),
  prayTimeoutMs: Schema9.number().default(8e3),
  statePath: Schema9.string().default("./data/mystic-state.json")
});
...
var Config10 = Schema10.object({
  dataDir: Schema10.string().default("./data"),
  maxCards: Schema10.number().default(200)
});
function nowId() {
...
var WikiStore = class {
  ...
  constructor(dataDir, username, maxCards) {
    ...
    this.path = join7(dataDir, `wiki-${username}.jsonl`);
...
var Config11 = Schema11.object({
  nearbyRadius: Schema11.number().default(28).description("\u6751\u6C11\u5B9E\u4F53\u89C6\u4E3A\u300C\u9644\u8FD1\u300D\u7684\u8DDD\u79BB\uFF08\u683C\uFF09"),
  maxMsgBuffer: Schema11.number().default(24).description("NPC \u8BDD\u8BED\u7F13\u51B2\u4E0A\u9650")
});
...
import { appendFileSync as appendFileSync4, mkdirSync as mkdirSync10, readFileSync as readFileSync11, readdirSync as readdirSync2, writeFileSync as writeFileSync7 } from "node:fs";
import { dirname as dirname8, join as join8, resolve as resolve10 } from "node:path";
...
var Config12 = Schema12.object({
  ...
  intervalMs: Schema12.number().default(5e3),
  baseUrl: Schema12.string().default("http://localhost:8890/v1"),
  apiKey: Schema12.string().default("sk-local"),
  model: Schema12.string().default("qwen3.8"),
  goal: Schema12.string().default("Explore the world, gather useful resources (wood, coal, stone), and stay alive."),
  persona: Schema12.string().default(""),
  historyDepth: Schema12.number().default(6),
  maxTokens: Schema12.number().default(1024),
  reasoningEffort: Schema12.string().default("none").description("none=\u6700\u5FEB\u53CD\u5E94\uFF08\u63A8\u8350\u7A7F\u8D8A\u8005\uFF09\uFF1Blow/medium/high \u9010\u6E10\u6DF1\u601D"),
  brainLogPath: Schema12.string().default("./data/mc-brain.log"),
  statusPath: Schema12.string().default("./data/status.json"),
  viewerPort: Schema12.number().default(3001),
  defectThreshold: Schema12.number().default(5),
  defectDir: Schema12.string().default("./data/defects"),
  defectNotifyUrl: Schema12.string().default("http://127.0.0.1:8088/api/console/chat"),
  defectNotifyAgent: Schema12.string().default("default")
});
...
function apply12(ctx2, config) {
  ...
  const episodicPath = (username) => join8(statusDir, `episodic-${username}.jsonl`);
  ...
  function appendEpisodic(username, text6) {
    try {
      appendFileSync4(episodicPath(username), JSON.stringify({ ts: (/* @__PURE__ */ new Date()).toISOString(), text: text6 }) + "\n", "utf8");
    } catch (err) {
  ...
  function notifyDefect(ticket, ticketPath) {
    ...
    fetch(config.defectNotifyUrl, {
      ...
      if (r.ok) {
        ...
        try {
          writeFileSync7(ticketPath, JSON.stringify(ticket, null, 2), "utf8");
        } catch {
        }
  ...
  function findOpenTicket(tool) {
    ...
    for (const f of files) {
      try {
        const t = JSON.parse(readFileSync11(join8(defectDir, f), "utf8"));
        if (t.tool === tool && t.status !== "closed") return { path: join8(defectDir, f), ticket: t };
      } catch {
      }
    }
    return null;
  ...
  function fileDefectTicket(bot, tool, goal) {
    try {
      mkdirSync10(defectDir, { recursive: true });
      ...
      if (existing) {
        ...
        writeFileSync7(existing.path, JSON.stringify(existing.ticket, null, 2), "utf8");
      ...
      const path = join8(defectDir, `${ticket.id}.json`);
      writeFileSync7(path, JSON.stringify(ticket, null, 2), "utf8");
...
\u8BF7\u603B\u7ED3\u8FD9\u4E2A\u76EE\u6807\u4E3A\u4EC0\u4E48\u53CD\u590D\u5931\u8D25\uFF0C\u4E0B\u6B21\u5E94\u8BE5\u6362\u4EC0\u4E48\u505A\u6CD5\uFF08\u6362\u8DEF\u7EBF/\u6362\u5DE5\u5177/\u5148\u505A\u51C6\u5907/\u653E\u5F03\u6B64\u76EE\u6807\uFF09\u3002`);
  ...
  function writeMapSnapshot(bot) {
    try {
      ...
      writeFileSync7(
        join8(statusDir, `map-${username}.json`),
        JSON.stringify({
          updatedAt: (/* @__PURE__ */ new Date()).toISOString(),
          r: R,
          cx: Math.floor(p.x),
  ...
  function writeStatus(bot) {
    try {
      ...
      writeFileSync7(
        join8(statusDir, `status-${username}.json`),
        JSON.stringify({
          updatedAt: (/* @__PURE__ */ new Date()).toISOString(),
          bot: {
            online: true,
  ...
  function brainLog(entry) {
    try {
      appendFileSync4(brainLogPath, JSON.stringify(entry) + "\n", "utf-8");
    } catch (err) {
  ...
  async function step() {
    ...
    try {
      ...
      if (process.env.MC_EYES_SHOT !== "0") {
        try {
          const shot = await captureFirstPerson(bot, join8(statusDir, "screenshots"));
          autoShot = shot.file;
        } catch {
        }
      }
...
import Schema13 from "@deepseek-ai/schemastery";
import { createServer } from "node:http";
import { closeSync, existsSync as existsSync9, mkdirSync as mkdirSync11, openSync, readSync, readdirSync as readdirSync3, readFileSync as readFileSync12, statSync, unlinkSync, writeFileSync as writeFileSync8 } from "node:fs";
import { join as join9, resolve as resolve11, sep } from "node:path";
var name13 = "mc-panel";
var inject12 = [];
var Config13 = Schema13.object({
  enabled: Schema13.boolean().default(true),
  host: Schema13.string().default("127.0.0.1"),
  port: Schema13.number().default(3200),
  dataDir: Schema13.string().default("./data"),
  /** which agent this panel shows; '' = auto-pick the freshest status-*.json */
  username: Schema13.string().default("")
});
...
function handleShot(dataDir, url, res) {
