...
function setLastImage(dataUrl, file = null, username2) {
  const s = slotOf(username2 ?? DEFAULT_SLOT);
  s.images = [{ dataUrl, file }];
  s.at = Date.now();
}
function setLastImages(images, username2) {
  if (images.length === 0) return;
  const s = slotOf(username2 ?? DEFAULT_SLOT);
  s.images = images;
  s.at = Date.now();
}
function takeLastImages(maxAgeMs = 6e4, maxImages = 2, username2) {
  const key = username2 ?? DEFAULT_SLOT;
...
var Config12 = Schema12.object({
  ...
  agents: Schema12.array(Schema12.object({
    username: Schema12.string().required(),
    sessionId: Schema12.string(),
    viewerPort: Schema12.number(),
    persona: Schema12.string(),
    goal: Schema12.string(),
    rules: Schema12.string(),
    provider: Schema12.string(),
    model: Schema12.string(),
    maxTokens: Schema12.number(),
    intervalMs: Schema12.number(),
    cwd: Schema12.string(),
    enabled: Schema12.boolean().default(true),
    autoSteer: Schema12.boolean()
  })),
  username: Schema12.string().default("HarnessBot"),
  sessionId: Schema12.string(),
  cwd: Schema12.string(),
  provider: Schema12.string().default("qwen-local"),
  model: Schema12.string().default("qwen3.8-27b"),
  maxTokens: Schema12.number(),
  intervalMs: Schema12.number().default(8e3),
  autoSteer: Schema12.boolean().default(true),
  dataDir: Schema12.string().default("./data"),
  viewerPort: Schema12.number().default(3200),
  persona: Schema12.string(),
  rules: Schema12.string(),
  goal: Schema12.string(),
  host: Schema12.string().default("127.0.0.1"),
  port: Schema12.number().default(25565),
  autoReconnect: Schema12.boolean().default(true),
  viewerEnabled: Schema12.boolean().default(true)
});
var DEFAULT_PERSONA = [
...
async function spawnTransmigrator(ctx2, entry, facade, deps) {
  ...
  let sessionId = entry.sessionId ?? `mc-${username2}`;
      ...
      try {
        ...
        for (const [names, label] of [
          ...
          for (const name15 of names) {
            const hb = bot.findBlock({ matching: (b) => !!b && b.name === name15, maxDistance: 8 });
            if (hb) {
...
\u4F60\u5F53\u524D\u7684\u76EE\u6807\uFF1A${goal}`
  ...
  const buildNudge = async () => {
    ...
    const imgs = svc?.saveImage ? takeLastImages(6e4, 2, username2) : [];
    const blocks = [];
  ...
  if (autoSteer) {
    ctx2.setInterval(() => {
      if (handle) {
        ...
        }).catch((err) => console.error(`[mc-session] buildNudge failed (${username2}):`, err));
      }
    }, intervalMs);
  }
}
