...
${loreHits.slice(0, 6).map((m) => `- ${m}`).join("\n")}` : "",
    ...
    let memoSaved = false;
    if (growth || resolve13) {
      const id = await ctx2.mcMemos.remember(username2, `\u3010\u6E38\u620F\u591C\u8FDB\u5316\u3011${growth}${resolve13 ? ` \u660E\u65E5\u4E4B\u5FC3\uFF1A${resolve13}` : ""}`);
      memoSaved = id !== null;
    }
...
var WikiStore = class {
  ...
  constructor(dataDir, username2, maxCards) {
    ...
    this.path = join6(dataDir, `wiki-${username2}.jsonl`);
...
var inject12 = ["agents", "timer", "mcbot"];
...
async function apply13(ctx2, config = {}) {
  ...
  const username2 = config.username ?? "HarnessBot";
  const sessionId = config.sessionId ?? `mc-${username2}`;
...
\u4F60\u5F53\u524D\u7684\u76EE\u6807\uFF1A${goal}`
  ...
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      await new Promise((resolve13) => setTimeout(resolve13, 3e3));
    }
  }
  ...
  const statusDir = resolve11(config.dataDir ?? "./data");
  try {
  ...
  const writeStatus = () => {
    try {
      ...
      writeFileSync7(join7(statusDir, `status-${username2}.json`), JSON.stringify(status));
    } catch (e) {
...
import { closeSync, existsSync as existsSync10, mkdirSync as mkdirSync10, openSync, readSync, readdirSync as readdirSync2, readFileSync as readFileSync12, statSync, unlinkSync, writeFileSync as writeFileSync8 } from "node:fs";
...
function collect(dataDir, username2) {
  ...
  try {
    const lines = readFileSync12(join8(dataDir, `wiki-${u}.jsonl`), "utf-8").split("\n").filter(Boolean);
