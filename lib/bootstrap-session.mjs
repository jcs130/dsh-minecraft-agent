...
${loreHits.slice(0, 6).map((m) => `- ${m}`).join("\n")}` : "",
    ...
    let memoSaved = false;
    if (growth || resolve11) {
      const id = await ctx2.mcMemos.remember(username2, `\u3010\u6E38\u620F\u591C\u8FDB\u5316\u3011${growth}${resolve11 ? ` \u660E\u65E5\u4E4B\u5FC3\uFF1A${resolve11}` : ""}`);
      memoSaved = id !== null;
    }
...
var WikiStore = class {
  ...
  constructor(dataDir, username2, maxCards) {
    ...
    this.path = join4(dataDir, `wiki-${username2}.jsonl`);
...
import { closeSync, existsSync as existsSync10, openSync, readSync, readdirSync as readdirSync2, readFileSync as readFileSync11, statSync } from "node:fs";
