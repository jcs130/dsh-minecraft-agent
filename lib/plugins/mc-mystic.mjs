...
function apply(ctx, config) {
  ...
  function parseInnate(message) {
    ...
    for (const name2 of INNATE_NAMES) {
      if (message.includes(name2)) return name2.replace(/术$/, "");
    }
    return null;
  }
  ...
  function ensurePassive(bot) {
    ...
    const capture = (message) => {
      ...
      if (grantText) {
        const name2 = grantText[1] || grantText[2] || grantText[3];
        if (name2 && !/法术|魔法|技艺|能力|恩典/.test(name2)) {
          const sb = ctx.get?.("mcSpellbook");
          sb?.grant?.(bot.username, name2);
          log(`goddess grant captured -> ${bot.username} + ${name2}`);
        }
      }
    };
    ...
    stopEnsure = ctx.setTimeout(() => {
      ...
      if (bot) {
        ...
        if (bot.entity && me && !store.get(me)) {
          if (Date.now() - lastQuery > 6e4 && innateAskCount < MAX_INNATE_ASKS) {
            innateAskCount++;
            lastQuery = Date.now();
            try {
...
` : "";
      ...
      if (chantNote) {
        const body = !reply ? `\uFF08\u4E16\u754C\u9759\u9ED8\u2014\u2014\u5929\u795E\u4F3C\u4E4E\u6CA1\u6709\u542C\u89C1\u4F60\u7684\u5492\u8BED\uFF0C\u6216\u5492\u8BED\u91CC\u6CA1\u6709\u5979\u4EEC\u8BA4\u8BC6\u7684\u6CD5\u672F\u5173\u952E\u8BCD\u3002\uFF09` : reply;
        return chantNote + "\n" + prefix + body;
      }
  ...
  ctx.tools.register(defineTool({
    ...
    description: "\u964D\u4E34\u4EEA\u5F0F\uFF1A\u4F60\u521A\u7A7F\u8D8A\u81F3\u6B64\u754C\uFF0C\u5973\u795E\u8D50\u4F60\u81EA\u9009\u4E00\u9879\u300C\u51FA\u751F\u5929\u8D4B\u300D\uFF08\u521D\u59CB\u6280\u80FD\uFF09\u3002\u5973\u795E\u4F1A\u5728\u516C\u5C4F\u5BA3\u8BFB\u5019\u9009\u6CD5\u672F\u6E05\u5355\uFF08\u683C\u5F0F\u5982\u300C1. \u5F52\u4E61 \u2014\u2014 \u56DE\u5BB6\u300D\uFF09\uFF0C\u8C03\u7528\u672C\u5DE5\u5177\u79C1\u8BED\u5BA3\u5E03\u4F60\u7684\u9009\u62E9\uFF08\u53EA\u6709\u5973\u795E\u542C\u89C1\uFF0C\u4E0D\u6270\u516C\u5C4F\uFF09\u3002\u53C2\u6570 skill \u586B\u8981\u9009\u7684\u6CD5\u672F\u540D\u6216\u7F16\u53F7\uFF08\u5982\u300C\u5F52\u4E61\u300D\u6216\u300C\u90091\u300D\uFF09\uFF0C\u9009\u4E00\u4E2A\u6700\u5951\u5408\u4F60\u4EBA\u8BBE/\u5904\u5883\u7684\uFF08\u6C42\u751F\u8005\u9009\u5F52\u4E61/\u5723\u6108/\u9971\u98DF\uFF0C\u597D\u6218\u8005\u9009\u4F20\u9001/\u9668\u77F3\u7B49\uFF09\u3002",
    parameters: {
      skill: {
        type: "string",
        required: true,
    ...
    execute: async (args) => {
      ...
      const reply = await waitForReply(
        bot,
        (from, msg, via) => (via === "whisper" || via === "chat") && from === config.godName && msg.includes(me) && !!parseInnate(msg),
        config.chantTimeoutMs
      );
      if (!reply) {
