...
function loadOverrides(dataDir) {
  try {
    const raw = JSON.parse(readFileSync2(join(dataDir, CONNECTION_FILE), "utf-8"));
    const out = {};
...
function createBotService(config, opts = {}) {
  ...
  function writeRuntime() {
    ...
    try {
      ...
      writeFileSync(join2(opts.dataDir, RUNTIME_FILE), JSON.stringify(payload, null, 2) + "\n", "utf-8");
    } catch {
    }
  }
...
function apply(ctx, config) {
  ...
  const ovFile = join2(dataDir, CONNECTION_FILE);
