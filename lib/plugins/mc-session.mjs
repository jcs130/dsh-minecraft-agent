...
import { existsSync as existsSync4, mkdirSync as mkdirSync6, readFileSync as readFileSync5, writeFileSync as writeFileSync4, watchFile as watchFile2, unwatchFile as unwatchFile2 } from "node:fs";
...
${loreHits.slice(0, 6).map((m) => `- ${m}`).join("\n")}` : "",
  ...
  let danmakuOffset = 0;
  try {
    danmakuOffset = existsSync4(danmakuPath) ? readFileSync5(danmakuPath, "utf-8").length : 0;
  } catch {
  }
  ctx.setInterval(() => {
    try {
      if (!existsSync4(danmakuPath)) return;
      const raw = readFileSync5(danmakuPath, "utf-8");
