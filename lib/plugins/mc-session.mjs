...
import { existsSync as existsSync3, mkdirSync as mkdirSync5, readFileSync as readFileSync4, writeFileSync as writeFileSync3, watchFile as watchFile2, unwatchFile as unwatchFile2 } from "node:fs";
...
async function apply(ctx, config = {}) {
  ...
  const ovFile = join6(dataDir, CONNECTION_FILE);
...
${hits}` : "";
  ...
  const directivesPath = join6(dataDir, `evolution-directives-${username}.json`);
  const tuningPath = join6(dataDir, `self-tuning-${username}.json`);
  const evolveStatePath = join6(dataDir, "evolve-state.json");
  try {
  ...
  try {
    ctx.tools.register(defineTool2({
      ...
      execute: async (args) => {
        ...
        saveJson(join6(proposalsDir, `${id}.json`), {
          id,
          username,
          title,
          motivation,
