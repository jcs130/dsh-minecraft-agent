...
import { existsSync as existsSync5, mkdirSync as mkdirSync7, readFileSync as readFileSync8, writeFileSync as writeFileSync6, watchFile as watchFile2, unwatchFile as unwatchFile2 } from "node:fs";
...
import { mkdirSync as mkdirSync4, watchFile, unwatchFile, writeFileSync as writeFileSync3 } from "node:fs";
...
function createPerception(deps) {
  ...
  let lastProgressAt = (() => {
    try {
      ...
      const raw = readFileSync2(join2(deps.dataDir, "progress-anchor.json"), "utf-8");
      const v = Number(JSON.parse(raw).lastProgressAt ?? 0);
  ...
  const status = () => {
    ...
    if (progressed) {
      ...
      if (deps.dataDir && Date.now() - progressWrittenAt > 6e4) {
        progressWrittenAt = Date.now();
        appendJsonl2(deps.dataDir, "progress-anchor.jsonl", { lastProgressAt, at: (/* @__PURE__ */ new Date()).toISOString() });
        try {
          writeFileSync2(join2(deps.dataDir, "progress-anchor.json"), JSON.stringify({ lastProgressAt }));
        } catch {
        }
      }
...
function loadOverrides(dataDir) {
  try {
    const raw = JSON.parse(readFileSync4(join4(dataDir, CONNECTION_FILE), "utf-8"));
    const out = {};
...
function createBotService(config, opts = {}) {
  ...
  function writeRuntime() {
    ...
    try {
      ...
      writeFileSync3(join5(opts.dataDir, RUNTIME_FILE), JSON.stringify(payload, null, 2) + "\n", "utf-8");
    } catch {
    }
  }
...
function defaultDeciderConfig(dataDir) {
  const fromFile = (() => {
    if (!dataDir) return "";
    try {
      return readFileSync5(join6(dataDir, "decider-key.txt"), "utf-8").trim();
    } catch {
      return "";
    }
  })();
...
// src/mc-audience.ts
import { existsSync as existsSync3, mkdirSync as mkdirSync5, readFileSync as readFileSync6, readdirSync, writeFileSync as writeFileSync4 } from "node:fs";
...
function createAudienceChannel(cfg = {}, deps = {}) {
  ...
  const loadSurfaced = () => {
    ...
    try {
      ...
      const j = JSON.parse(readFileSync6(cfg.statePath, "utf-8"));
  ...
  const saveSurfaced = () => {
    try {
      ...
      writeFileSync4(cfg.statePath, JSON.stringify({ windowId, surfaced: [...surfaced] }), "utf-8");
    } catch {
    }
  };
  return {
    ...
    noteProfile(source, senderKey, summary, append) {
      ...
      try {
        ...
        const p = join7(viewersDir, source, `${senderKey}.md`);
        const old = existsSync3(p) ? readFileSync6(p, "utf-8") : "";
...
function loadSkins(dataDir) {
  try {
    const raw = JSON.parse(readFileSync7(resolve3(dataDir, "skins.json"), "utf-8"));
    return {
      presets: raw.presets ?? {},
...
async function actUnstick(bot, ms) {
  ...
  const ahead = (() => {
    try {
      return b.blockAt(new Vec34(Math.floor(p.x + 1), Math.floor(p.y), Math.floor(p.z + 1)));
    } catch {
      return null;
    }
  })();
  const aheadBelow = (() => {
    try {
      return b.blockAt(new Vec34(Math.floor(p.x + 1), Math.floor(p.y) - 1, Math.floor(p.z + 1)));
    } catch {
      return null;
    }
  })();
  const unsafe = (x) => !!x?.name && /lava|water|fire|cactus|powder_snow/i.test(x.name);
  const noFloor = !aheadBelow?.name || aheadBelow.name === "air";
  if (unsafe(ahead) || unsafe(aheadBelow) || noFloor) return;
...
function saveJson(path, data) {
  try {
    ...
    writeFileSync6(path, JSON.stringify(data, null, 2), "utf-8");
  } catch {
  }
}
var CAUSE_MAP = [
...
${loreHits.slice(0, 6).map((m) => `- ${m}`).join("\n")}` : "",
  ...
  let danmakuOffset = 0;
  try {
    danmakuOffset = existsSync5(danmakuPath) ? readFileSync8(danmakuPath, "utf-8").length : 0;
  } catch {
  }
  ctx.setInterval(() => {
    try {
      if (!existsSync5(danmakuPath)) return;
      const raw = readFileSync8(danmakuPath, "utf-8");
