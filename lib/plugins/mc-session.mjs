...
import { existsSync as existsSync2, mkdirSync as mkdirSync4, readFileSync as readFileSync3, writeFileSync as writeFileSync2, watchFile as watchFile2, unwatchFile as unwatchFile2 } from "node:fs";
...
import { mkdirSync as mkdirSync3, watchFile, unwatchFile, writeFileSync } from "node:fs";
...
function appendJsonl(dir, file, obj) {
  try {
    ...
    try {
      if (statSync(p).size > 8 * 1024 * 1024) appendFileSync(p, "");
    } catch {
    }
...
function createPerception(deps) {
  ...
  const arm = (botRaw) => {
    ...
    bot.on("health", () => guard(() => {
      const hp = n(bot.health, 20);
      const delta = hp - lastHp;
      if (lastHp >= 0 && delta < 0) {
        const th = threatState(bot, 16);
        const src = th.nearest ? `\uFF08\u6700\u8FD1\u5A01\u80C1 ${th.nearest}\uFF09` : "";
        push("\u5916\u611F\xB7\u89E6", `\u4F60\u88AB\u51FB\u4E2D ${delta}\uFF08${Math.round(hp)}/20\uFF09${src}`);
      }
      if (lastHp >= 0 && delta > 0) push("\u5185\u611F", `\u4F60\u6062\u590D\u4E86 ${delta} \u70B9\u751F\u547D\uFF08${Math.round(hp)}/20\uFF09`);
      lastHp = hp;
    }));
    bot.on("breath", () => guard(() => {
      const o = n(bot.oxygenLevel, 20);
      if (o <= 5 && o < lastOxygen) push("\u5185\u611F", `\u{1FAE7}\u6C27\u6C14\u9AA4\u964D ${o}/20\uFF08\u6C34\u4E0B\u4F1A\u6EBA\u6B7B\uFF0C\u7ACB\u523B\u4E0A\u6D6E\u6362\u6C14\uFF09`);
      if (o > lastOxygen && lastOxygen >= 0 && lastOxygen <= 10) push("\u5185\u611F", "\u{1FAE7}\u547C\u5438\u6062\u590D\u6B63\u5E38");
      lastOxygen = o;
    }));
    ...
    bot.on("entityHurt", (...a) => guard(() => {
      ...
      if (e === me()) {
        ...
        push("\u5916\u611F\xB7\u89E6", `\u4F60\u88AB\u51FB\u4E2D${by}${where && where !== "\u811A\u4E0B0\u683C" ? `\uFF08${where}\uFF09` : ""}`);
        return;
      }
      if (!e.name) return;
      if (!where) return;
  ...
  const status = () => {
    ...
    if (region !== lastRegion) {
      lines.push(`\u{1F4CD}\u4F60\u79FB\u5230 (${Math.round(pos.x)}, ${Math.round(pos.y)}, ${Math.round(pos.z)})`);
      lastRegion = region;
    ...
    if (food !== lastFood || food <= 8) lines.push(`${clock.stamp} \u{1F357}\u9971\u98DF ${food}/20${food <= 8 ? " \u26A0\u9965\u997F" : ""}`);
    if (oxygen <= 10) {
    ...
    lines.push(`${clock.stamp} \u3010\u6001\u52BF\u3011\u4F60\u5728 (${Math.round(pos.x)}, ${Math.round(pos.y)}, ${Math.round(pos.z)})\uFF5C\u8FD1\u5904\u654C\u5BF9\u602A ${threat.count} \u4E2A${threat.nearest ? `\uFF08\u6700\u8FD1 ${threat.nearest}${threat.ranged ? "\xB7\u8FDC\u7A0B" : ""}\uFF09` : ""}${threat.rangedAny && !threat.ranged ? "\xB7\u53E6\u6709\u8FDC\u7A0B" : ""}\uFF5C\u5728\u7EBF\u65C5\u4EBA ${trav.total}${loaded >= 0 ? `\uFF5C\u611F\u77E5 ${loaded} \u533A\u5757` : ""}`);
    ...
    const travDesc = trav.total ? `\u8EAB\u8FB9\u65C5\u4EBA ${trav.nearby.length ? trav.nearby.map((v) => `${v.dir}${v.d}\u683C`).join("\u3001") : "\u65E0"}\uFF5C\u8FDC\u5904\u5728\u7EBF ${trav.faraway}${trav.unknown ? `\uFF5C\u65B9\u4F4D\u672A\u660E ${trav.unknown}` : ""}` : "\u8FD9\u4E2A\u4E16\u754C\u6B64\u523B\u53EA\u6709\u4F60\u4E00\u4E2A\u65C5\u4EBA";
    sig = {
      hp,
      food,
      oxygen,
...
async function apply(ctx, config = {}) {
  ...
  ctx.on("agent/pre-step", async (_payload, next) => {
    try {
      ...
      if (submitted && config.goalCooldownMs > 0) {
        await new Promise((resolve4) => setTimeout(resolve4, config.goalCooldownMs));
      }
    } catch {
    }
    return next();
  ...
  const ovFile = join5(dataDir, CONNECTION_FILE);
...
async function spawnTransmigrator(ctx, entry, facade, deps) {
  ...
  const goalFile = resolve3(dataDir, "active-goals.json");
  ...
  if (persistedGoal) {
    ...
    log(`\u5DF2\u6062\u590D\u4E0A\u6B21\u7684\u5F53\u524D\u76EE\u6807\uFF1A${activeGoal.slice(0, 40)}`);
  }
...
${hits}` : "";
  ...
  const directivesPath = join5(dataDir, `evolution-directives-${username}.json`);
  const tuningPath = join5(dataDir, `self-tuning-${username}.json`);
  const evolveStatePath = join5(dataDir, "evolve-state.json");
  try {
  ...
  try {
    ctx.tools.register(defineTool2({
      ...
      execute: async (args) => {
        ...
        saveJson(join5(proposalsDir, `${id}.json`), {
          id,
          username,
          title,
          motivation,
...
\u884C\u52A8\u4F18\u5148\uFF08\u66F4\u91CD\u8981\uFF09\uFF1A\u522B\u505C\u5728\u539F\u5730\u7A7A\u60F3\u6216\u53CD\u590D"\u786E\u8BA4\u5F53\u524D\u72B6\u6001"\u3002\u5148\u7528 mc_see \u770B\u6E05\u811A\u4E0B\u4E0E\u56DB\u5468\uFF08\u7B2C\u4E00\u4EBA\u79F0\u753B\u9762\uFF09\uFF0C\u518D\u7528\u773C\u775B\u89C2\u5BDF\u7ED3\u679C\u51B3\u5B9A\u53BB\u54EA\uFF0C\u7136\u540E\u7528 mc_goto \u8D70\u5411\u76EE\u6807\u70B9\u3001\u7528 mc_collect / mc_dig / mc_build / mc_tunnel \u7B49\u4E16\u754C\u5DE5\u5177\u771F\u6B63\u52A8\u624B\u5B9E\u73B0\u4F60\u7684\u76EE\u6807\u3002\u6BCF\u4E00\u6B65\u90FD\u671D\u7740\u300C\u3010\u4F60\u5F53\u524D\u7684\u76EE\u6807\u3011\u300D\u63A8\u8FDB\uFF0C\u800C\u4E0D\u662F\u539F\u5730\u5FAA\u73AF\u3002`
  ...
  function solidAt(bot, x, y, z) {
    const b = bot.blockAt(new Vec34(x, y, z));
  ...
  try {
    ...
    log(`mc_set_goal \u6CE8\u518C\u8DF3\u8FC7\uFF1A${err instanceof Error ? err.message : err}`);
  }
  try {
    const spellReflect = () => {
      try {
    ...
    log(`mc_reflect_skills \u6CE8\u518C\u8DF3\u8FC7\uFF1A${err instanceof Error ? err.message : err}`);
  }
  try {
    const spellList = () => {
      try {
    ...
    log(`mc_skills \u6CE8\u518C\u8DF3\u8FC7\uFF1A${err instanceof Error ? err.message : err}`);
  }
  try {
    ...
    log(`mc_spell_detail \u6CE8\u518C\u8DF3\u8FC7\uFF1A${err instanceof Error ? err.message : err}`);
  }
  if (autoSteer) {
    if (!resumed) {
