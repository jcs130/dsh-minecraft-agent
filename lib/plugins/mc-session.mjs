...
import { existsSync as existsSync5, mkdirSync as mkdirSync7, readFileSync as readFileSync6, writeFileSync as writeFileSync5, watchFile as watchFile2, unwatchFile as unwatchFile2 } from "node:fs";
...
// src/mc-audience.ts
import { existsSync as existsSync3, mkdirSync as mkdirSync5, readFileSync as readFileSync4, readdirSync, writeFileSync as writeFileSync3 } from "node:fs";
...
function createAudienceChannel(cfg = {}, deps = {}) {
  ...
  const hasProfile = (source, key) => {
    ...
    try {
      return existsSync3(join6(viewersDir, source, `${key}.md`));
    } catch {
      return false;
    }
  };
  ...
  const loadSurfaced = () => {
    ...
    try {
      ...
      const j = JSON.parse(readFileSync4(cfg.statePath, "utf-8"));
  ...
  const saveSurfaced = () => {
    try {
      ...
      mkdirSync5(join6(cfg.statePath, ".."), { recursive: true });
  ...
  return {
    ...
    readProfile(source, senderKey) {
      ...
      try {
        const p = join6(viewersDir, source, `${senderKey}.md`);
    ...
    noteProfile(source, senderKey, summary, append) {
      ...
      try {
        mkdirSync5(join6(viewersDir, source), { recursive: true });
        const p = join6(viewersDir, source, `${senderKey}.md`);
        const old = existsSync3(p) ? readFileSync4(p, "utf-8") : "";
    ...
    profileCounts() {
      ...
      try {
        ...
        for (const ent of readdirSync(viewersDir, { withFileTypes: true })) {
          ...
          let n = 0;
          for (const f of readdirSync(join6(viewersDir, ent.name))) if (f.endsWith(".md")) n++;
          if (n) out[ent.name] = n;
        }
      } catch {
      }
...
// src/agent-store.ts
import { existsSync as existsSync4, mkdirSync as mkdirSync6, readFileSync as readFileSync5, writeFileSync as writeFileSync4 } from "node:fs";
import { resolve as resolve3 } from "node:path";
function loadSkins(dataDir) {
  try {
    const raw = JSON.parse(readFileSync5(resolve3(dataDir, "skins.json"), "utf-8"));
    return {
      presets: raw.presets ?? {},
...
var REFLEXES = [surface, escape, eat];
function pickReflex(s) {
  const critical = s.danger >= 4;
  if (s.agentBusy && !critical) return null;
...
async function apply(ctx, config = {}) {
  ...
  const ovFile = join7(dataDir, CONNECTION_FILE);
...
${hits}` : "";
  ...
  const hotspotsPath = join7(dataDir, "death-hotspots.json");
  const proposalsDir = join7(dataDir, "evolution-proposals");
  const directivesPath = join7(dataDir, `evolution-directives-${username}.json`);
  const tuningPath = join7(dataDir, `self-tuning-${username}.json`);
  const evolveStatePath = join7(dataDir, "evolve-state.json");
  try {
  ...
  try {
    ctx.tools.register(defineTool2({
      ...
      execute: async (args) => {
        ...
        saveJson(join7(proposalsDir, `${id}.json`), {
          id,
          username,
          title,
          motivation,
...
${loreHits.slice(0, 6).map((m) => `- ${m}`).join("\n")}` : "",
  ...
  const audience = createAudienceChannel(
    {
      viewersDir: join7(dataDir, "viewers"),
      statePath: join7(dataDir, "audience-state.json"),
    ...
    void (async () => {
      try {
        ...
        const input = {
          ...
          safeDirection: (dx, dz) => {
            try {
              const ahead = bot.blockAt(new Vec34(Math.floor(p.x + dx * 2), Math.floor(p.y) - 1, Math.floor(p.z + dz * 2)));
              const at = bot.blockAt(new Vec34(Math.floor(p.x + dx * 2), Math.floor(p.y), Math.floor(p.z + dz * 2)));
  ...
  const danmakuPath = join7(dataDir, "danmaku.jsonl");
  let danmakuOffset = 0;
  try {
    danmakuOffset = existsSync5(danmakuPath) ? readFileSync6(danmakuPath, "utf-8").length : 0;
  } catch {
  }
  ctx.setInterval(() => {
    try {
      if (!existsSync5(danmakuPath)) return;
      const raw = readFileSync6(danmakuPath, "utf-8");
