...
${npcLines}`);
      ...
      try {
        ...
        const mini = (x, z) => {
          const above = bot.blockAt(new Vec33(x, fy + 1, z));
          const g = bot.blockAt(new Vec33(x, fy, z));
          const below = bot.blockAt(new Vec33(x, fy - 1, z));
          const na = above?.name ?? "";
          const ng = g?.name ?? "";
          const nb = below?.name ?? "";
        ...
        parts.push(`\u5468\u56F4\u5730\u5F62(\u5317\u2191,5\xD75,\u4F60\u25CF): ${mapRow}`);
      } catch {
      }
