...
export function apply(ctx: Context, config: Config) {
  ...
  ctx.tools.register(defineTool({
    name: 'mc_remember',
    description:
      '把重要经历写入你的长期记忆（跨重启持久、可语义检索）。值得记的：新发现（"在(-200,64,180)发现裸露钻石"）、重要交易（"和遇到的行商用32铁锭换11绿宝石"）、与人交往（"一位旅人送我一张床，很感激"）、教训（"夜晚沼泽边被苦力怕炸过，别再去"）、计划与承诺（"答应遇到的村民明天送他小麦"）。写自然语言完整句子（含关键细节：坐标/人名/物品/数量），别写流水账，一次一件事。',
    parameters: {
      content: {
        type: 'string',
        required: true,
